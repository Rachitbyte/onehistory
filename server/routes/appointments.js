...
// GET /api/appointments - List with Filters (Step 4 & 3.4)
router.get('/', (req, res) => {
    const user = req.user;
    ...
    if (view === 'upcoming') {
        ...
        // Past or Terminal appointments (History Tab)
        // Logic: Either explicitly terminal status OR time is in past.
        // Actually simpler: Everything NOT in 'Upcoming' set?
        // Let's be explicit for clarity + performance index friendliness.
        // History = (Terminal Status) OR (Active Status BUT in Past)
...
// POST /api/appointments - Create Request (Step 4)
router.post('/', async (req, res) => {
    console.log("!!! SERVER HIT !!! POST /api/appointments"); // MARKER
    const { targetId, startTime, duration, type, reasonTags, reasonText, caseId, rescheduledFromId } = req.body;
    const user = req.user;

    // console.log("Appointment Request Body:", req.body); // DEBUG
    ...
    //     return res.status(400).json({ error: `DEBUG_SERVER: Missing required fields: ${missing.join(', ')}` });
    // }

    // FORCE ERROR TO TEST CONTROL
    return res.status(400).json({ error: "DEBUG: I HAVE CONTROL" });
    ...
    // SQLite doesn't natively do date math easily in node without limits, but we use the query logic above.
    // For JS side:
...
// PATCH /api/appointments/:id/status - State Management (Step 2 & 6)
router.patch('/:id/status', async (req, res) => {
    const { id } = req.params;
    const { status, reason } = req.body; // reason used for cancel note if needed
    const user = req.user;

    const allowedTransitions = {
        'REQUESTED': ['CONFIRMED', 'CANCELLED'],
        'CONFIRMED': ['COMPLETED', 'CANCELLED', 'NO_SHOW', 'RESCHEDULED'],
        'RESCHEDULED': ['CANCELLED'] // New appt is created; old one goes here? Logic dependent.
    };

    // Reschedule flow is special - it creates a new appointment usually. 
    // If status is 'RESCHEDULED', we assume frontend handled the creation of new one?
    // User requirement: "Missing reschedule linkage... Add rescheduled_from_id"
    // So "Reschedule" action -> 1. Mark old as RESCHEDULED ? 2. Create new one.
    // Ideally this endpoint just updates status.
